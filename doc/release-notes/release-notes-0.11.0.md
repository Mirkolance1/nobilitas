Nobilitas Core version 0.11.0 is now available from:

  <https://nobilitas.org/bin/nobilitas-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer NOBI/kB fee to be
...
For example, add the following to `nobilitas.conf`:
...
related to the blockchain in the nobilitas system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Nobilitas Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Nobilitas Core will 
...
For an example script see [nobilitas-submittx](https://github.com/laanwj/nobilitas-submittx).
...
- #5334 `cf87536` libnobilitasconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new nobilitas-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only NobilitasAmountField
- #5753 `0f44672` Add nobilitas logo to about screen
...
- #5809 `46bfbe7` Add nobilitas-cli man page
...
- #5849 `41259ca` contrib/init/nobilitasd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Nobilitas-Qt.app to Nobilitas-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for nobilitas-tx)
