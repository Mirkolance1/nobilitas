...
const char * const NOBILITAS_CONF_FILENAME = "nobilitas.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "nobilitasd -foo=bar
...
#else
    const char* pszModule = "nobilitas";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Nobilitas
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Nobilitas
    // Mac: ~/Library/Application Support/Nobilitas
    // Unix: ~/.nobilitas
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nobilitas";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Nobilitas";
#else
    // Unix
    return pathRet / ".nobilitas";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", NOBILITAS_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Nobilitas Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Nobilitas Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Nobilitas Core developers";
    }
