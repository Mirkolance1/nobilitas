...
int main()
{
    std::cout << "nobilitasconsensus version: " << nobilitasconsensus_version() << std::endl;
    ...
    auto op0Result = nobilitasconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, nobilitasconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
