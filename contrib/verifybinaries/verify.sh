...
###   bitcoincore.org and nobilitas.org and compares them.
...
if [ $? -ne 0 ]; then
   echo "nobilitas.org failed to provide signature file, but bitcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "nobilitas.org and bitcoincore.org signature files were not equal?"
