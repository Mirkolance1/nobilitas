#!/usr/bin/env bash
# Copyright (c) 2016 The Nobilitas Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   nobilitascore.org and nobilitas.org and compares them.
...
WORKINGDIR="/tmp/nobilitas_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://nobilitascore.org"
HOST2="https://nobilitas.org"
BASEDIR="/bin/"
VERSIONPREFIX="nobilitas-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'nobilitas-',
...
if [ $? -ne 0 ]; then
   echo "nobilitas.org failed to provide signature file, but nobilitascore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "nobilitas.org and nobilitascore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Nobilitas Core binary release signing key installed?"
   fi
